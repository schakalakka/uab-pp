...
//    }
    ...
    for (int t = 1; t < T + 1; ++t) {
        for (int x = 1; x < X; ++x) {
            if (t % 3 == 1) U[x][2] = 2 * (1 - L) * U[x][1] + L * U[x + 1][1] + L * U[x - 1][1] - U[x][0];
            if (t % 3 == 2) U[x][0] = 2 * (1 - L) * U[x][2] + L * U[x + 1][2] + L * U[x - 1][2] - U[x][1];
            if (t % 3 == 1) U[x][1] = 2 * (1 - L) * U[x][0] + L * U[x + 1][0] + L * U[x - 1][0] - U[x][2];
